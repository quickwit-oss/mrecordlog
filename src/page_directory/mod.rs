...
impl Directory {
    pub fn create_or_open(path: &Path, len: u64) -> io::Result<()> {
        ...
        match create_or_open {
            CreateOrOpen::Created => Directory::create(file, num_pages),
            CreateOrOpen::Opened => Directory::open(file, num_pages),
        }
    }
    ...
    fn open(file: File, num_pages: usize) -> io::Result<()> {
        todo!()
    }
