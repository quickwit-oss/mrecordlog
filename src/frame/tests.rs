...
fn test_frame_simple() {
    ...
    frame_writer
        .write_frame(FrameType::First, &b"abc"[..], &mut wrt)
        .unwrap();
    frame_writer
        .write_frame(FrameType::Middle, &b"de"[..], &mut wrt)
        .unwrap();
    frame_writer
        .write_frame(FrameType::Last, &b"fgh"[..], &mut wrt)
        .unwrap();
    wrt.persist(PersistAction::Flush).unwrap();
...
fn test_frame_corruption_in_payload() -> io::Result<()> {
    let mut buf: Vec<u8> = {
        ...
        frame_writer.write_frame(FrameType::First, &b"abc"[..], &mut wrt)?;
        frame_writer.write_frame(FrameType::Middle, &b"de"[..], &mut wrt)?;
        wrt.into()
    };
    buf[8] = 0u8;
...
fn repeat_empty_frame_util(repeat: usize) -> Vec<u8> {
    ...
    for _ in 0..repeat {
        frame_writer
            .write_frame(FrameType::Full, &b""[..], &mut wrt)
            .unwrap();
    }
    wrt.into()
}
